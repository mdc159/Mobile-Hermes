...
| Username | u0_a369 |
| Password | 1234567 |
| Status | ⚠️ Requires wake + sshd restart |
...
| Available Tools | llama-completion, llama-simple-chat, llama-server, llama-bench, etc. |

### Installed models

| Model | File | Size |
|---|---|---|
| Hermes-3-Llama-3.2-3B | Hermes-3-Llama-3.2-3B.Q4_K_M.gguf | ~2 GB |
| Hermes-2-Pro-Llama-3-8B | (templates only) | - |

### Hermes-3 test results

| Check | Status |
|---|---|
| Model loads | ✅ Successful |
| GPU support | ❌ No GPU detected |
| Memory management | ⚠️ Context auto-reduced to ~50k tokens |
| Phone RAM | ~4–6 GB (estimated) |
---
...
- Creates the virtual environment with `python -m venv`
...
- `python`: runtime + virtual environment support
...
`$PREFIX/bin` is already on `PATH` in Termux, so this makes the `hermes` command persist across new shells without re-activating the virtual environment every time.
...
Use the Termux path with the stdlib virtual environment + pip instead:
